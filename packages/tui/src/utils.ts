...
// From pi-mono: a table-driven view of the same problem loop probes for.
// Kept because the checks below reference it; loop's calibration still has
// the final say on how many cells a mark actually gets.
...
function graphemeWidth(segment: string): number {
    ...
    // Some marks occupy cells even without a base character — but HOW MANY is
    // the thing loop probes the terminal for at startup, so this scales by the
    // measured width rather than assuming one cell each. With the default
    // calibration (1) this is exactly pi-mono's behaviour.
    if (terminalSpacingMarkRegex.test(segment)) {
        return [...segment].length * spacingMarkCellWidth;
    }
