...
describe("CombinedAutocompleteProvider", () => {
    ...
    // Needs the real `fd` binary — skipped where it isn't installed
    // (bun:test has no node:test options object, so skipIf replaces { skip }).
    describe.skipIf(!isFdInstalled)("fd @ file suggestions", () => {
        let rootDir = "";
        let baseDir = "";
        let outsideDir = "";
