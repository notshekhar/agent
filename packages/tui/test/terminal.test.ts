...
import { setKittyProtocolActive } from "../src/keys";
import { normalizeAppleTerminalInput, ProcessTerminal } from "../src/terminal";

// bun:test has no node:test mock.timers — the negotiation-timing tests below
// use real (short) timers against the 150ms fragment-flush timeout instead.
...
describe("ProcessTerminal Kitty keyboard protocol negotiation", () => {
    ...
    it("activates Kitty mode for non-zero negotiated flags", () => {
        ...
        try {
            ...
            // Divergence from upstream: loop's stop() always emits BOTH resets
            // (kitty pop + modifyOtherKeys off) regardless of tracked flags — a
            // Ctrl+C racing the startup negotiation can leave a flag stale, and
            // the resets are idempotent. Upstream expects no >4;0m here.
            assert.equal(harness.writes.includes("\x1b[>4;0m"), true);
        } finally {
            harness.cleanup();
        }
    });
    ...
    it("tracks split Kitty confirmation", async () => {
        ...
        try {
            harness.send("\x1b[?7");
            await sleep(10); // well under the 150ms fragment flush
