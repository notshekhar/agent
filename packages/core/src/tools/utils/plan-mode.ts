 ...
 * tool stripping, but flippable mid-turn (the enter_plan_mode tool) and
 * mid-session (the TUI's agent cycle). Subagents inherit the gate: their
 * ToolContext carries the parent turn's sessionId, so delegation can't widen
 * write access while planning.
 *
 * State is process-local by design: plan mode is an interactive-session
 * concept, owned by the TUI (which clears it on implement/exit) and the
 * enter_plan_mode tool (which sets it with user approval).
 */
