...
import { EXTENSION_API_VERSION, manifestSection, requiredApiRange } from "./api";
...
export function resolveEntry(pkgDir: string, manifest: ExtensionManifest): string {
    const rel = manifestSection(manifest)?.entry ?? manifest.module ?? manifest.main ?? "index.ts";
...
export function isCompatible(manifest: ExtensionManifest): boolean {
    const want = requiredApiRange(manifest);
...
export function assertLoadable(pkgDir: string, manifest: ExtensionManifest): void {
    if (!isCompatible(manifest)) {
        throw new Error(`requires ${PRODUCT_NAME} API ${requiredApiRange(manifest)}, host is ${EXTENSION_API_VERSION}`);
    }
