...
#
# Sources are `loop-icon.svg` and `loop-mark-small.svg`, both generated by
# `make-icons.py` — edit the colours or the wordmark there, re-run it, then run
# this. Nothing here is hand-drawn, so the whole identity regenerates.
#
...
for f in "$src" "$small_src"; do
  [[ -f "$f" ]] || { echo "missing $f — run make-icons.py first" >&2; exit 1; }
done
