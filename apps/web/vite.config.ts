...
// Imported by relative path on purpose: tsconfig `paths` do not apply when Vite
// loads its own config, so `@loop/shared/devProxy` would not resolve here.
import { DEV_PROXIED_PATH_PREFIXES } from "./src/loop/shared/devProxy.ts";

// Upstream read Clerk/relay/tracing keys out of a repo .env here. loop's desktop
// app has no cloud side, so every one of those resolves to empty.
